...
def chatbot_ask():
    ...
    # Example: Show all reminders
        ...
        if reminders:
            ...
            # Old reminders column
            reply += '<div style="min-width:180px;"><b style="color:#b91c1c">Old Reminders</b><ul>'
            if old_reminders:
                reply += ''.join(f'<li style="color:#b91c1c">{r.content} on {r.date.strftime('%Y-%m-%d')} at {r.time.strftime('%H:%M')}</li>' for r in old_reminders)
            else:
