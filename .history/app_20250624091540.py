...
def answer_from_db(user_message):
    ...
    # Subjects or history
    if 'subject' in user_message or 'history' in user_message:
        subjects = SubjectName.query.all()
        if not subjects:
            return 'No subjects found.'
        lines = [f'<li><a href="/view_subject_details/{s.namesub}" target="_blank">{s.namesub} (Staff: {s.staffname}, Note: {s.note})</a></li>' for s in subjects]
        return '<ul>' + ''.join(lines) + '</ul>'
    # Staff
