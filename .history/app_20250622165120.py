...
class Remainder(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_email = db.Column(db.String(100), nullable=False)  # Add this line
...
def dashboard():
    ...
    remainders = Remainder.query.filter_by(user_email=user_email).order_by(Remainder.date.asc()).all()
...
def add_remainder():
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect(url_for('dashboard'))
        remainder = Remainder(user_email=user_email, content=content, date=date, time=time, link=link)
