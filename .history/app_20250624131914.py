...
def chatbot_ask():
    ...
    if 'subject' in question or 'all subjects' in question:
        subjects = SubjectName.query.all()
        if subjects:
            reply = 'Here are your current subjects:<ul>' + ''.join(f'<li>{s.namesub} (Staff: {s.staffname})</li>' for s in subjects) + '</ul>'
        else:
    ...
    # Example: Show all staff
        ...
        staff = SubjectName.query.with_entities(SubjectName.staffname).distinct().all()
        if staff:
            reply = 'Staff members:<ul>' + ''.join(f'<li>{s.staffname}</li>' for s in staff) + '</ul>'
        else:
            reply = 'No staff found.'
    # Example: Show all reminders
    elif 'reminder' in question:
        from datetime import date
        today = date.today()
        reminders = Remainder.query.order_by(Remainder.date.asc()).all()
        if reminders:
            old_reminders = [r for r in reminders if r.date < today]
            new_reminders = [r for r in reminders if r.date >= today]
            reply = '<div style="display:flex;gap:24px;flex-wrap:wrap">'
            # Old reminders column (show only one, rest hidden)
            reply += '<div style="min-width:180px;max-width:220px;">'
            reply += '<b style="color:#b91c1c">Old Reminders</b><ul id="old-reminders-list" style="list-style:disc inside;max-height:120px;overflow:hidden;padding-left:0;">'
            if old_reminders:
                first = old_reminders[0]
                reply += f'<li style="color:#b91c1c">{first.content} on {first.date.strftime('%Y-%m-%d')} at {first.time.strftime('%H:%M')}</li>'
                if len(old_reminders) > 1:
                    reply += f'<li id="show-more-old-reminders" style="color:#b91c1c;cursor:pointer;text-decoration:underline;list-style:none;">Show more...</li>'
                    # Hidden old reminders
                    for r in old_reminders[1:]:
                        reply += f'<li class="old-reminder-extra" style="color:#b91c1c;display:none;">{r.content} on {r.date.strftime('%Y-%m-%d')} at {r.time.strftime('%H:%M')}</li>'
            else:
                reply += '<li style="color:#b91c1c">None</li>'
            reply += '</ul>'
            reply += '</div>'
            # New reminders column
            reply += '<div style="min-width:180px;"><b style="color:#166534">Upcoming Reminders</b><ul>'
            if new_reminders:
                reply += ''.join(f'<li style="color:#166534">{r.content} on {r.date.strftime('%Y-%m-%d')} at {r.time.strftime('%H:%M')}</li>' for r in new_reminders)
            else:
                reply += '<li style="color:#166534">None</li>'
            reply += '</ul></div>'
            reply += '</div>'
            # Add JS for show more (use setTimeout to ensure DOM is ready)
            reply += '''<script>\nsetTimeout(function(){\nvar btn=document.getElementById('show-more-old-reminders');\nif(btn){\nbtn.onclick=function(){\nvar extras=document.querySelectorAll('.old-reminder-extra');\nfor(var i=0;i<extras.length;i++){extras[i].style.display='list-item';}\nbtn.style.display='none';\n};\n}}, 100);<\/script>'''
        else:
