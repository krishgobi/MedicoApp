...
@app.route("/")
def dashboard():
    # Query the database for subjects, staff, and exams
    subjects = SubjectName.query.all()
    num_subjects = len(subjects)
    staff_list = list({s.staffname for s in subjects})
    # Dummy upcoming_exams for now (customize as needed)
    upcoming_exams = []
    return render_template('dashboard.html', num_subjects=num_subjects, staff_list=staff_list, upcoming_exams=upcoming_exams)
...
def add_subject():
    # This route is triggered when the Add Subject form is submitted (plus symbol clicked)
    # It adds the subject to the database and redirects to the dashboard
