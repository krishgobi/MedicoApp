...
def answer_from_db(user_message):
    ...
    if 'patient' in user_message:
        ...
        if not patients:
            return '<b>Patients:</b><br><i>No patients found yet.</i>'
        lines = [f'<li><a href="/view_history?patient={p[0]}" target="_blank">{p[0]}</a></li>' for p in patients]
        return '<b>Patients:</b><ul>' + ''.join(lines) + '</ul>'
    # Disease
...
def chatbot_ask():
    ...
    # Example: Show all reminders
        ...
        if reminders:
            reply = 'Upcoming reminders:<ul>' + ''.join(f'<li>{r.content} on {r.date.strftime('%Y-%m-%d')} at {r.time.strftime('%H:%M')}</li>' for r in reminders) + '</ul>'
        else:
    ...
    # Example: Show all patients from History table
    elif 'patient' in question:
        patients = History.query.all()
        if patients:
            reply = 'Recent patients:<ul>' + ''.join(f'<li>{p.patientname} - {p.disease}</li>' for p in patients) + '</ul>'
        else:
